...
- The guest OS must be booting completely
  ...
  - emergency or single user mode is not a valid troubleshooting state
...
- The string "@commandline" meaning the package was installed from a downloaded file, and does not state where it came from
- No output (blank)
...
- If the agent service is not set to 'enabled' this will be called out.  Note that the 'preset' for the systemd unit is disabled, but packages will usually enable it in their post-install.
- For best functionality the 'AutoUpgrade' flag in the waagent.conf should be set to True, this allows the extension handler to upgrade to match the Azure infrastructure.  The base Agent package is not altered
- Most Azure features that touch the guest OS operate as an extension.  If this flag in the waagent.conf is disabled it will be flagged.
...
#### Networking
While networking does not strictly cause the agent to not be online, all communication may fail if the networking of the guest is misconfigured
